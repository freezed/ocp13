...
Après avoir réaliser, entre autre, [un jeu en 2D][p3], [un chat-bot][p5] et [un outil de recherche d'aliments de substitution][p11] explorant notamment l'écosystème [Python][wikipython], le [TDD][wikitdd] ou l'[Intégration continue][wikici], me voici enfin arrivé à la dernière étape du parcours _[développeur·se d'application Python][dapy]_ : développer un projet _libre_ avec une _portée sociale_.
...
_Véronique_ et _Serges_ sont travailleurs indépendants (dans la suite du doc. les `utilisateurs`, c'est eux). Nous nous rencontrons dans la [communauté de travail][cordee] que nous fréquentons à Lyon. Pour leurs activités respectives ils sont à la recherche d'un outil qui leur permettrai de gérer aisément leur _relation client_ : prospection, suivi, prise de contact, etc.
...
* service en ligne ([SaaS][wikisaas]), en gardant à l'esprit qu'un usage _hors réseau_ sera certainement implenté à l'avenir
* les données seront réparties dans différents _«objets»_ :
    - des `clients` auxquels seront associés des `affaires`
    - aux `affaires` et `clients` pourront être associés à des `notes`
    - des `étiquettes` permettront une organisation souple et transversale de ces informations en s'associant à tout ou partie des objets définis ci dessus. Une étiquette pourrait représenter : un _groupe de client_, une _entreprise_, un _secteur professionnel_, un _statut_, etc.
* les `clients` seront cloissonnés à l'`utilisateur` qui les aura créés, en gardant à l'esprit qu'un partage des `clients` entre les `utilisateurs` du service sera implémenté à l'avenir
